...
#   Version : 2004091501
#   Use for : 
#             1. for HD to delete forward E-Mail
#   Host : x
#  ----------------------------------
use IO::Socket;
...
die "./del_forward.pl old_account old_domain\n" if (scalar(@ARGV)!=2);

# Forwarding E-Mail by HD Rmail Web Page
&delforwarding($old_mail);
...
sub delforwarding {
    ...
    if (!$buf =~ /\+OK/) {
         ...
         print $sock_target "delforward\n";
    }
    $buf=<$sock_target>;chomp($buf); $buf=~s/\r//g;
    if ($buf =~ /\+OK/) {
        print $sock_target "$old_account\n";
        $buf=<$sock_target>;chomp($buf); $buf=~s/\r//g;
        if ($buf =~ /\+OK/) {
             print $sock_target "quit\n";
             print STDOUT "Info: $old_mail cancel forwarding OK!\n\n";
        } else {
             print $sock_target "quit\n";
